...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO work_packages (project_id, name, status) VALUES ($1, $2, $3) RETURNING *',
      [project_id, name, status || 'pending']
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE work_packages SET name = $1, status = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3 RETURNING *',
